...
This script assumes you have correct values in your .env_semantic or exported environment variables, especially for SEMANTIC_SEARCH_TERMS, EMAIL_PASSWORD, SENDER_EMAIL, and RECIPIENT_EMAIL.

Make sure all dependencies are installed and version-compatible.

Detailed logs will be written both to console and neurocell_agent_semantic.log to facilitate diagnosis.

If you see any zero scores again, check the log for explicit error messages on model loading or embedding instructions.Here is your complete, fixed script (just copy-paste this into neurocell_agent_semantic_fixed.py). It addresses loading, term, and semantic scoring errors, and all code blocks from your original have been included and improved where needed for robust semantic filtering and logging. See the comments for all enhancements.
...
# -------------------------
# Load .env_semantic if local run
load_dotenv(".env_semantic")
# -------------------------
DB_FILE = os.getenv("DB_FILE", "neurocell_database_semantic.db")
SENDER_EMAIL = os.getenv("SENDER_EMAIL")
...
try:
    model = SentenceTransformer('all-MiniLM-L6-v2')
    # Sanity check
    logger.info(f"Test embedding: {model.encode('test').shape}")
except Exception as e:
...
def semantic_filter(docs: List[Dict[str, Any]], terms: List[str], threshold: float) -> List[Dict[str, Any]]:
    logger.info(f"Semantic terms used: {terms}")
    if not model:
        logger.warning("Semantic model not loaded — skipping semantic filtering.")
        for d in docs:
            d['semantic_score'] = 0.0
    ...
    try:
        term_embeddings = model.encode(terms, convert_to_tensor=True)
        logger.info(f"Encoded {len(terms)} terms. Embeddings shape: {term_embeddings.shape}")
    except Exception as e:
        logger.error(f"Error encoding semantic terms: {str(e)}")
        for d in docs:
            d['semantic_score'] = 0.0
        return docs
    ...
    for doc in docs:
        ...
        try:
            ...
            logger.error(f"Error embedding doc '{title[:50]}': {str(e)}")
            max_score = 0.0
        doc['semantic_score'] = round(max_score, 4)
        if max_score >= threshold:
            filtered_docs.append(doc)
